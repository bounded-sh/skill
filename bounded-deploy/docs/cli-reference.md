...
| `login` | Log the CLI into your Bounded **web account** (the canonical identity; no key involved). By default it opens the hosted sign-in page, completes Authorization Code + PKCE through a temporary loopback callback, stores refreshable credentials in `~/.bounded/web-session.json`, and selects `account.keySource:"web"` for the current project unless `BOUNDED_ACCOUNT_FILE` supplies the process account. Use `--email <addr>` or `--no-browser` for terminal OTP when a browser is unavailable. **Headless agents:** run `bounded login --email <email>` with stdin held open, relay the 6-digit code from the user, then feed it to stdin. Never ask for or embed a reusable credential. | `bounded login` |
...
| `account` / `account use` | Show the effective account or set this project's account source in `bounded.json`: global, project, profile, env, or web. A process-only `BOUNDED_ACCOUNT_FILE` override is reported by `account --json`; unset it before `account use`. See [accounts.md](accounts.md#process-account-selection-for-automation). | `bounded account use --web` |
