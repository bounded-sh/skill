...
| `wallet fund <wallet>` | Add simulated SOL, USDC, or another mint to any wallet inside one `realtime_offchain` app. Requires the app owner, an admin, or a developer. `--mint` defaults to `SOL`; pass `USDC` or a Solana mint address. This never creates real onchain assets. | `bounded wallet fund <wallet> --app-id <id> --mint USDC --amount 1000` |
