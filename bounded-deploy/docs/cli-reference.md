...
| `site versions` | List retained (restorable) canonical site versions with timestamps, the live version, and pin state. The newest 10 successful deploys are kept automatically; pinned versions are kept in addition; older versions age out. | `bounded site versions --app-id <id> --json` |
