...
| Share an app / add a collaborator / grant admin, deploy, or billing rights | `bounded share <email-or-wallet> --role admin\|developer\|viewer\|billing --app-id <id>` (owner-only). If no current identity is the owner, find which login IS the owner (owner wallet → its linked email → `bounded login --email <it>`): [docs/access-playbook.md](docs/access-playbook.md#1b-owner-only-operations-share-roster-transfer-find-which-login-is-the-owner). Do NOT hunt for an allowlist in app code; the control plane governs access. Capability matrix in the **bounded-backend** skill's access-control doc. |
...
- If policy deployment reports `deploy_in_progress` with an `operationId`, do not repeat a normal deploy.
  ...
  Each retained runtime publication also has its own finite autonomous recovery deadline.
  If the initiating request disappears, the control plane finishes an already acknowledged safe publication or abandons it, keeps the last committed release serving, and frees the app for a later normal deploy.
...
- If the project GENERATES `policy.json` (a `build-policy` script, `npm run policy`), regenerate before every verify/deploy and confirm the file changed. A generator that prints to stdout does nothing without `> policy.json` — verify and deploy then "succeed" on the stale bytes and every conclusion drawn from that run is about the old policy.
- `bounded deploy` succeeding does not mean the new revision is serving yet. Before measuring runtime behavior (tests, probes, "did my rule change take?"), poll `bounded apps inspect --app-id <id> --json` until the policy revision advances; testing against the old runtime produces confident, wrong findings.
