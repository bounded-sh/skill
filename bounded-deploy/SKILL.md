...
| Recover an app policy deploy that reports `deploy_in_progress`, inventory apps after a project-limit error, or prove the exact active policy/runtime publication after deploy | Run only the exact owner-visible `recoveryCommand`, or use `bounded apps list --json` and `bounded apps inspect --app-id <id> --json`; see [docs/cli-reference.md](docs/cli-reference.md#recover-an-in-progress-policy-deploy) and [exact release provenance](docs/cli-reference.md#exact-release-provenance). |
...
| `409`, `deploy_in_progress`, `operationId`, `recoveryCommand`, `--recover-operation` | [docs/cli-reference.md](docs/cli-reference.md#recover-an-in-progress-policy-deploy) - the verified app owner resumes only the exact retained operation with unchanged policy inputs. |
...
- If policy deployment reports `deploy_in_progress` with an `operationId`, do not repeat a normal deploy. The verified app owner must run the exact emitted `recoveryCommand` with the unchanged policy file and inputs.
