...
validation with **zero issues**. The literal `"false"` rules are the
**intentional always-deny idiom** for immutable/server-authoritative collections.

> A literal `"false"` rule is the **intended append-only / server-authoritative
> deny idiom**, and `bounded deploy` accepts it. The `"false"` rules in the
> examples below are correct and deployable as written.
...
What the invariants buy you: membership is the *closure* of the owner plus
...
alone enforce write *integrity*, not read isolation: with a permissive `read: "@user.id
...
- An order's `amountUsd` and `seller` must match the referenced **active**
  ...
  the enforced cap is over the real price, not a buyer-supplied number.
...
A cap is only as trustworthy as the field it sums. If the buyer could
write `amountUsd` freely, they could place a real order for a $500 listing with
`amountUsd: 0` and the $5,000/day ceiling would mean nothing. So the
...
the `input_rate_cap` invariant. What no backend can cure — a script firing *legal*
