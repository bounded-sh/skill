...
Every launched page serves under `script-src 'self'; object-src 'none'; base-uri 'none'; worker-src 'self' blob:` whatever the app declared, so scripts must ship inside the frozen release; the other lists (`connect`, `img`, `font`, `style`, `embeddedBy`) compile exactly as declared.
