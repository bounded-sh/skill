...
- **Keep the auth defaults for most apps.** Do not add `authMode` or `auth.wallets` merely to enable wallets. Turnkey is the sole embedded-wallet implementation and the default is Turnkey-native auth with eager provisioning, so a completed email/social login carries a real `@user.address` alongside its stable `@user.id`. Add explicit auth config to opt out (`auth.wallets: false`) or retain the legacy hosted login mode - and to opt IN to wallet login. Wallet/keypair SIGN-IN (SIWS/SIWE, `BOUNDED_PRIVATE_KEY`, CI, an agent's QA session) is OFF until the policy sets `auth: { wallets: true }` explicitly, and is otherwise refused with `403 wallet_login_disabled`. That is a separate switch from the embedded-wallet provisioning default above. Browser guests and phone-only sessions are exceptions. See [embedded-wallets.md](../bounded-onchain/docs/embedded-wallets.md).
