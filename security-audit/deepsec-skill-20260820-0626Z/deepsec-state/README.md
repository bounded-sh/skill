...
Currently configured project: `bounded-skill` (target: `../../..`).
...
original init command. Checkpoints in `data/bounded-skill/setup/setup-state.json`
...
`--project-id bounded-skill` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/bounded-skill/`.
