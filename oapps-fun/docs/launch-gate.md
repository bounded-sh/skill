...
- **The launched host serves under a script floor.** Every launched page gets
  ...
  `worker-src 'self' blob:` in its Content-Security-Policy, even if
  ...
  browser refuses it on every load. Bundle it instead.
- **The launched origin is pinned to its own backend.** A request from the
  launched origin (`https://<slug>.openapps.xyz`, a venue apex host, or the
  local `openapps.localhost` zone) may only name the app the router serves
  there. Realtime, its WebSocket handshake, and function invokes answer `403`
  with `origin_app_mismatch` when the page names any other app id, however
  that id was produced. The creator's own hosts are not launched hosts and are
  not pinned.
