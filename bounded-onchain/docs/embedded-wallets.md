...
- Wallet-page labels and the Crossmint staging test stablecoin are separate from the policy compiler's `@TokenPlugin.USDC` constant.
  That TokenPlugin constant is mainnet-only and must not be used for a devnet TokenPlugin flow.
  Use an app-created devnet mint for Bounded token-operation tests.
