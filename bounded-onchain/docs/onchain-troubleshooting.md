...
| `pump_create_reserved_creator: createToken creator <pubkey> is a reserved platform signer` (also for `createTokenV2`) | The `creator` you passed is the platform's own sponsor wallet or attestation server. The creator pays Pump.fun's `Create` and, as a wallet, keeps a transaction-level signature, so naming the platform signer would make Bounded pay for and sign the app's launch. The refusal is fail-closed and fires before any transaction is built. | Pass the acting user (`@user.address`), the escrow (`@contract.address`), or an app account id as the `creator`, and fund it for the Create cost as the row above describes. |
