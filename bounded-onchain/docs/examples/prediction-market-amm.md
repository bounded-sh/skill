...
```json
{
  ...
  "pmPositions/$marketId/holders/$holderId/sells/$orderId": {
    ...
    "rules": {
      "read": "true",
      "create": "@user.address != null && @newData.yesIn > 0 && get(/pmMarkets/$marketId) != null && get(/pmResolves/$marketId) == null && get(/pmMarkets/$marketId).expiryTs > @time.now && get(/pmPositions/$marketId/holders/$holderId) != null && get(/pmPositions/$marketId/holders/$holderId).owner == @user.address && get(/pmPositions/$marketId/holders/$holderId).yesBalance >= @newData.yesIn && @PredictionMarketPlugin.getCollateralOutAmm(@newData.yesIn, get(/pmMarkets/$marketId).collateralReserve, get(/pmMarkets/$marketId).yesSupply) > 0 && @TokenPlugin.getBalance($marketId, @TokenPlugin.SOL) >= @PredictionMarketPlugin.getCollateralOutAmm(@newData.yesIn, get(/pmMarkets/$marketId).collateralReserve, get(/pmMarkets/$marketId).yesSupply)",
      "update": "false",
      "delete": "false"
    },
    "hooks": { "onchain": {
      "create": "@TokenPlugin.transfer($marketId, @user.address, @TokenPlugin.SOL, @PredictionMarketPlugin.getCollateralOutAmm(@newData.yesIn, get(/pmMarkets/$marketId).collateralReserve, get(/pmMarkets/$marketId).yesSupply)) && @DocumentPlugin.updateField(/pmMarkets/$marketId, 'collateralReserve', get(/pmMarkets/$marketId).collateralReserve - @PredictionMarketPlugin.getCollateralOutAmm(@newData.yesIn, get(/pmMarkets/$marketId).collateralReserve, get(/pmMarkets/$marketId).yesSupply)) && @DocumentPlugin.updateField(/pmMarkets/$marketId, 'yesSupply', get(/pmMarkets/$marketId).yesSupply + @newData.yesIn) && @DocumentPlugin.updateField(/pmPositions/$marketId/holders/$holderId, 'yesBalance', get(/pmPositions/$marketId/holders/$holderId).yesBalance - @newData.yesIn)"
    } }
  },
  "pmResolves/$marketId": {
    "onchain": true,
    ...
    "rules": {
      "read": "true",
      "create": "@user.address != null && get(/pmMarkets/$marketId) != null && @user.address == get(/pmMarkets/$marketId).creator && (@newData.outcome == 'YES' || @newData.outcome == 'NO') && @newData.winningSupply == null",
      "update": "false",
      "delete": "false"
    },
    "hooks": { "onchain": {
...
- **Batched writes share pre-state.** Rules read pre-transaction state, so two sells for the same position bundled in one `setMany` each pass the balance gate independently; the pinned redeem cannot be duplicated this way, but sells can, which is why sell payouts are AMM-priced against the reserve rather than guaranteed 1:1. Do not weaken the pinned-id pattern when adapting this policy.
- **Trust boundary.** The creator is the oracle. Resolution is only accepted between expiry and the end of the claim window, so a late resolution cannot time-bar redemptions and then sweep the pot. A creator who never resolves strands the pot permanently (redeem and sweep both require the resolution document) - add a refund path if that risk is unacceptable for your market.
