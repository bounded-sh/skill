...
- **Rules stay pure; hooks move funds.** Authorization is entirely in `rules` (the enforced plane); all mutating plugin calls sit in `hooks.onchain.create/delete` and revert atomically with the document write.
