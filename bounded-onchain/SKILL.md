...
- **Default onchain login = default Turnkey email/social auth + optional wallet login, side by side.** Do not add `authMode` or `auth.wallets` for the normal path. Turnkey is the sole embedded-wallet implementation, and Turnkey-native auth with eager provisioning is already the default. Supported email/social users have `@user.address` when login completes. Add bring-your-own **wallet login** (`walletLogin`) only for users who already hold a Solana wallet. Deploy `"auth": { "wallets": true }` for any EXTERNAL-KEYPAIR session - wallet login, a server-SDK keypair client, or the CLI keypair data lane - since the issuer refuses a session for a key it did not provision. Use explicit auth policy only for that, to opt out, or to retain the legacy hosted login mode. See [docs/embedded-wallets.md -> recommended login](docs/embedded-wallets.md#the-recommended-login-for-an-onchain-app).
