...
- Mainnet is a real target, not a placeholder: the program is live on mainnet-beta. Creating a mainnet app needs the platform's mainnet creation key, and the app is owned on-chain by the creator's wallet **immutably** - so it must be created from the machine holding that key, and it can never be ownership-transferred. See [docs/onchain.md](docs/onchain.md).
